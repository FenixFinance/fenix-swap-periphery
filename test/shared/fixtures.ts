...
import FenixFactory from '../../buildV2/FenixFactory.json'
import IFenixPair from '../../build/IFenixPair.json'
...
import FenixRouter01 from '../../build/FenixRouter01.json'
import FenixMigrator from '../../build/FenixMigrator.json'
import FenixRouter02 from '../../build/FenixRouter.json'
...
export async function addPair(
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IFenixPair.abi), provider).connect(wallet)
...
export async function getPair(provider: Web3Provider, [wallet]: Wallet[], factoryV2: Contract, tokenA: Contract, tokenB: Contract) {
  const pairAddress = await factoryV2.getPair(tokenA.address, tokenB.address)
  return new Contract(pairAddress, JSON.stringify(IFenixPair.abi), provider).connect(wallet)
}
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, FenixFactory, [wallet.address])

  // deploy routers
  const router01 = await deployContract(wallet, FenixRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, FenixRouter02, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, FenixMigrator, [factoryV1.address, router01.address], overrides)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IFenixPair.abi), provider).connect(wallet)
